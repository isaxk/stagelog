<script lang="ts">
	...
	import ShowListItemWrapper from '$lib/components/log-list/show-list-item-wrapper.svelte';
	...
	const search = debounce(async () => {
		const { data, error } = await supabase.from('shows').select('*').textSearch('name', query, {
			type: 'phrase'
		});
		results = data ?? [];
	});
...
{#if mobile}
	<div class="z-10 flex h-16 items-center justify-center rounded-t-lg bg-zinc-100 p-4 drop-shadow">
		<div class="flex w-full max-w-screen-sm items-center">
			<div class="w-40"></div>
			<div class="flex-grow text-center font-semibold">Adding {selectedItems.length} Shows</div>
...
{:else}
	<div class="sticky top-0 z-20 flex h-24 w-full items-center bg-zinc-50 p-4">
		<div class="flex flex-grow flex-col">
			<div class="font-serif text-4xl font-medium lg:text-3xl">Add Shows</div>
