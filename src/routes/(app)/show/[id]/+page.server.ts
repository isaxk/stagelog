...
export const load: LayoutServerLoad = async ({ params, locals: { supabase, safeGetSession } }) => {
    ...
    const logResponse = await supabase.from('log_entries').select('*').eq('show_id', params.id).order('date', {ascending:false});
    const logs = logResponse.data ?? [];

    console.log(logs.map((o:Tables<'log_entries'>)=>o.user_id));
    const profilesRes = await supabase.from('profiles').select('*').in('id', logs.map((o:Tables<'log_entries'>)=>o.user_id));
